...
$checkService="select * from slider where id=".$_POST['serviceid'];
$resultService=$conn->query($checkService);

if($resultService->num_rows==1){
    $deleteService="delete from slider where id=".$_POST['serviceid'];
    if($conn->query($deleteService)){
    $output['status']='true';
    $output['message']='Slider deleted!';
    }
    else{
    $output['status']='false';
    $output['message']='failed to delete Slider!';
    }
}
else{
    $output['status']='false';
    $output['message']='Service does not exist!';
}
